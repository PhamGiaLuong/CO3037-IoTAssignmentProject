...
* **Mô tả:** Xây dựng Web Server chạy trên ESP32 đóng vai trò là Captive Portal, cung cấp giao diện (SPA) để giám sát và cấu hình thiết bị.
* **Chi tiết tính năng:**
    * **Dashboard:** Hiển thị trạng thái kết nối, kết quả dự đoán TinyML, giá trị cảm biến (nhiệt độ, độ ẩm) và biểu đồ cập nhật theo thời gian thực.
    * **Điều khiển (Controls):** Giao diện nút gạt bật/tắt thiết bị ngoại vi (Buzzer, Relay) và công tắc chuyển đổi sang chế độ WiFi (Station Mode).
    * **Cấu hình hệ thống (Settings):** Quản lý và lưu trữ thông tin mạng (AP, WiFi), thông số kết nối CoreIoT Cloud, và cấu hình các ngưỡng (min/max) của cảm biến xuống bộ nhớ Flash.
